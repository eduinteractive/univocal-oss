...
const EventAccreditation = () => {
    ...
    const createAttendeeMutation = useMutation({
        mutationFn: SAPI.EVENT.PUBLIC.createEventAttendee,
        onSuccess: () => {
            setIsLoading(false);
            setIsFinished(true);
        },
    ...
    const handleSubmit = () => {
        if (!firstName || !lastName || !email || !eventId) {
            NotificationHandler.showError(t('PAGES.EVENT.ACCREDITATION.ERRORS.FILL_ALL_FIELDS'));
            return;
        }
    ...
    if (!eventQuery.data?.config?.accreditation.enabled) {
        return (
            <HTTP_403
                title={t('PAGES.EVENT.ACCREDITATION.NO_ACCESS_TITLE')}
                description={t('PAGES.EVENT.ACCREDITATION.NO_ACCESS_DESCRIPTION')}
            />
        );
    }
                    ...
                    {isFinished && (
                        <>
                            <Text size="sm" ta="center" fw="bold" pb="md">
                                {t('PAGES.EVENT.ACCREDITATION.THANKS')}
                            </Text>
                            {eventQuery.data.config.toc.enabled && (
                                <Anchor
                                    href={`/event-program/${eventId}`}
                                    ta="center"
                                    pb="md"
                                >
                                    {t('PAGES.EVENT.PROGRAM.PROGRAM_LINK')}
                                </Anchor>
                            )}
                        </>
                    )}
            ...
            {!isFinished && !isLoading && (
                <Card withBorder p="xl" radius="md" shadow="sm" w="100%">
                    ...
                    <Text c="dimmed" size="sm" fw="bold">
                        {t('PAGES.EVENT.ACCREDITATION.SECTION_TITLE')}
                    </Text>
                    <EDITextInput
                        label={t('PAGES.EVENT.ACCREDITATION.FIELDS.FIRST_NAME')}
                        placeholder={t('PAGES.EVENT.ACCREDITATION.FIELDS.FIRST_NAME_PLACEHOLDER')}
                        value={firstName}
                        onChange={(e) => setFirstName(e.currentTarget.value)}
                        required
                    />
                    <EDITextInput
                        label={t('PAGES.EVENT.ACCREDITATION.FIELDS.LAST_NAME')}
                        placeholder={t('PAGES.EVENT.ACCREDITATION.FIELDS.LAST_NAME_PLACEHOLDER')}
                        value={lastName}
                        onChange={(e) => setLastName(e.currentTarget.value)}
                        required
                    />
                    <EDITextInput
                        label={t('PAGES.EVENT.ACCREDITATION.FIELDS.EMAIL')}
                        placeholder={t('PAGES.EVENT.ACCREDITATION.FIELDS.EMAIL_PLACEHOLDER')}
                        value={email}
                        onChange={(e) => setEmail(e.currentTarget.value)}
                        required
                    />
                    {eventQuery.data?.config?.accreditation.fields.map(
                        (field) => {
                    ...
                    <Button w="100%" mt="md" onClick={handleSubmit}>
                        {t('PAGES.EVENT.ACCREDITATION.SUBMIT')}
                    </Button>
                </Card>
            )}
        </Container>
